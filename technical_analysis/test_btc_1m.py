...
# Cargar los datos
data = pd.read_csv("./data/btc_project_test.csv").dropna()
...
# Calcular el indicador MACD con las ventanas optimizadas
...
# Generar señales de compra y venta con los umbrales optimizados
...
for i, row in technical_data.iterrows():
    ...
    # Abrir nuevas posiciones si hay señal de compra
    if row.BUY_SIGNAL:
    ...
    # Evaluar las posiciones activas y actualizar el valor del portafolio
    positions_value = sum(
...
# Estrategia pasiva para comparación
...
# Imprimir el valor final del portafolio activo
