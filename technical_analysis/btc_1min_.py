...
# Load data
data_btc = pd.read_csv("./data/btc_project_1m_train.csv").dropna()
...
for combination in all_combinations:
    study = optuna.create_study(direction='maximize')
    study.optimize(func=lambda trial: profit_calculator.profit(trial, data_btc, combination), n_trials=30)
...
best_outcome_json = json.dumps(best_outcome_btc_1min, indent=4)

# Saving file
with open("best_outcome_btc_1min.txt", "w") as file:
